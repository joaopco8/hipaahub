...
const FAQItem: React.FC<{ question: string; answer: string }> = ({ question, answer }) => {
  ...
  return (
    <div className="border-b border-gray-100 last:border-0">
      ...
      >
        <span className={`text-slate-800 text-base md:text-xl transition-colors ${isOpen ? 'text-cisco-blue font-normal' : 'font-normal hover:text-cisco-blue'}`}>
          {question}
        </span>
        {isOpen ? <ChevronUp size={20} className="text-cisco-blue" /> : <ChevronDown size={20} className="text-gray-300" />}
      </button>
      {isOpen && (
        <div className="pb-8 text-gray-500 text-sm md:text-base font-light leading-relaxed whitespace-pre-wrap animate-reveal">
          {answer}
        </div>
      )}
    </div>
...
const FAQSection: React.FC = () => {
  ...
  return (
    <section id="support" className="py-24 bg-white border-b">
      <div className="max-w-4xl mx-auto px-4 md:px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl lg:text-[48px] font-light text-slate-800 leading-tight">Frequently Asked Questions</h2>
          <h3 className="text-xl font-normal text-cisco-blue mt-4">Everything you need to know about HIPAA Hub.</h3>
        </div>
