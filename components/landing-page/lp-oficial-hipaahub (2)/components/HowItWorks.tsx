...
const StepCard: React.FC<{ 
  ...
  <div className="bg-white p-10 border border-gray-100 flex flex-col h-full hover:shadow-2xl transition-all duration-500 group">
    <div className="flex items-center justify-between mb-8">
      ...
      <div className="text-4xl font-light text-gray-100 group-hover:text-cisco-blue/10 transition-colors">0{number}</div>
    </div>
    <div className="text-xs font-semibold text-cisco-blue mb-2 tracking-widest">Step {number}</div>
    <h4 className="text-2xl font-light mb-6 text-cisco-navy">{title}</h4>
    <ul className="space-y-3 mb-8 flex-grow">
      {list.map((item, i) => (
        <li key={i} className="flex items-start text-base text-gray-600 font-light">
    ...
    <div className="pt-6 border-t border-gray-50 flex items-center justify-between">
       <span className="text-xs font-semibold text-gray-400 tracking-widest">Phase timeline</span>
       <span className="text-sm font-semibold text-cisco-navy flex items-center gap-2"><Clock size={14} /> {duration}</span>
    </div>
  </div>
);
...
const HowItWorks: React.FC = () => {
  return (
    <section id="how-it-works" className="py-24 bg-white border-t border-b">
      <div className="max-w-7xl mx-auto px-4 md:px-12">
        <div className="text-center mb-24">
          <h2 className="text-4xl lg:text-[54px] font-light text-cisco-navy leading-[1.2] mb-6">
        ...
        <div className="mt-20 bg-cisco-navy p-12 text-center text-white flex flex-col items-center">
           ...
           <h3 className="text-3xl font-light mb-10">Timeline: Audit-ready within 2 weeks.</h3>
           <button className="bg-cisco-blue text-white px-12 py-5 text-sm font-bold hover:bg-white hover:text-cisco-navy transition-all flex items-center gap-3">
