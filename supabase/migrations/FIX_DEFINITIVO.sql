-- ============================================================================
-- HIPAA Guard - FIX DEFINITIVO - Remover TODAS as versões e recriar
-- ============================================================================
-- Execute este script no Supabase SQL Editor
-- ============================================================================

-- PASSO 1: Remover TODAS as versões possíveis da função (incluindo com diferentes assinaturas)
...
-- PASSO 2: Garantir que a coluna existe
...
-- PASSO 3: Criar a função CORRETA do zero
...
BEGIN
  ...
  v_metadata := jsonb_build_object(
    ...
    'number_of_locations', CASE WHEN p_data->>'number_of_locations' IS NOT NULL THEN (p_data->>'number_of_locations')::integer ELSE NULL END,
    'multi_state_operations', CASE WHEN p_data->>'multi_state_operations' IS NOT NULL THEN (p_data->>'multi_state_operations')::boolean ELSE NULL END,
    'remote_workforce', CASE WHEN p_data->>'remote_workforce' IS NOT NULL THEN (p_data->>'remote_workforce')::boolean ELSE NULL END,
    'ehr_system', p_data->>'ehr_system',
    'email_provider', p_data->>'email_provider',
    'cloud_storage_provider', p_data->>'cloud_storage_provider',
    'uses_vpn', CASE WHEN p_data->>'uses_vpn' IS NOT NULL THEN (p_data->>'uses_vpn')::boolean ELSE NULL END,
  ...
  -- Remove null values
  v_metadata := v_metadata - (SELECT array_agg(key) FROM jsonb_each(v_metadata) WHERE value = 'null'::jsonb OR value IS NULL);
  ...
  -- CHECK IF ORGANIZATION EXISTS - CRITICAL: Cast v_user_id to ensure UUID type
  SELECT o.id INTO v_existing_org_id
  FROM public.organizations o
  WHERE o.user_id::uuid = v_user_id::uuid  -- Explicit cast to ensure UUID comparison
  LIMIT 1;
  ...
  ELSE
    ...
    SET
      ...
      updated_at = v_now
    WHERE o.user_id::uuid = v_user_id::uuid  -- Explicit cast
