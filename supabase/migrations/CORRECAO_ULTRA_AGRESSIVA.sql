-- ============================================================================
-- CORREÇÃO ULTRA AGRESSIVA - Força correção de TUDO
-- ============================================================================
-- Execute este script no Supabase SQL Editor
-- ============================================================================

-- PASSO 1: Desabilitar RLS temporariamente para fazer correções
ALTER TABLE organizations DISABLE ROW LEVEL SECURITY;

-- PASSO 2: Converter user_id para UUID (FORÇADO)
DO $$
BEGIN
    ...
    IF EXISTS (
        ...
        -- Backup dos dados antes de converter (caso necessário)
...
-- PASSO 4: Remover TODAS as funções antigas (mais agressivo)
DO $$
DECLARE
    r record;
BEGIN
    ...
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE (proname LIKE '%upsert_organization%' OR proname LIKE '%organization%')
...
-- PASSO 5: Garantir coluna metadata
ALTER TABLE organizations 
...
-- PASSO 6: Criar função RPC ULTRA ROBUSTA
...
BEGIN
  ...
  -- FORÇAR tipo UUID explicitamente
  v_user_id := (v_user_id::text)::uuid;
  
  -- EXTRACT METADATA (simplificado)
  v_metadata := COALESCE(
    jsonb_build_object(
  ...
  -- CHECK IF ORGANIZATION EXISTS - FORÇAR cast em ambos os lados
  SELECT o.id::uuid INTO v_existing_org_id
  FROM public.organizations o
  WHERE (o.user_id::text)::uuid = (v_user_id::text)::uuid
  LIMIT 1;
  ...
  IF v_existing_org_id IS NULL THEN
    ...
    VALUES (
      gen_random_uuid(),
      (v_user_id::text)::uuid,  -- Cast duplo para garantir
      NULLIF(p_data->>'name', ''),
  ...
  ELSE
    ...
    SET
      ...
      updated_at = v_now
    WHERE (o.user_id::text)::uuid = (v_user_id::text)::uuid  -- Cast duplo
    RETURNING id INTO v_org_id;
  END IF;
...
-- PASSO 7: Recriar RLS Policies CORRETAS
ALTER TABLE organizations ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own organization" ON organizations
  FOR SELECT USING ((user_id::text)::uuid = (auth.uid()::text)::uuid);

CREATE POLICY "Users can insert own organization" ON organizations
  FOR INSERT WITH CHECK ((user_id::text)::uuid = (auth.uid()::text)::uuid);

CREATE POLICY "Users can update own organization" ON organizations
  FOR UPDATE USING ((user_id::text)::uuid = (auth.uid()::text)::uuid);

CREATE POLICY "Users can delete own organization" ON organizations
  FOR DELETE USING ((user_id::text)::uuid = (auth.uid()::text)::uuid);
...
BEGIN
  ...
  RAISE NOTICE '✅ Coluna user_id verificada/convertida para UUID';
END $$;
