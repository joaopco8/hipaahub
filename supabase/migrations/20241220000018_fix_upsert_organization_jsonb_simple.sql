-- Fix upsert_organization_jsonb function - versão simplificada que funciona
-- Remove validação de auth.uid() que pode estar causando problemas
-- A função usa SECURITY DEFINER para bypassar RLS, então confiamos no p_user_id passado
