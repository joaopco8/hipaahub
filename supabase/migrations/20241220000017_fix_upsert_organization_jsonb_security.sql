-- Fix upsert_organization_jsonb function to add security validation
-- This ensures the function validates user_id matches authenticated user
-- even though it uses SECURITY DEFINER to bypass RLS
...
BEGIN
  ...
  -- This is a security check even though SECURITY DEFINER bypasses RLS
  IF v_authenticated_user_id IS NULL THEN
    RAISE EXCEPTION 'User must be authenticated to create or update organization';
  END IF;
  
  IF p_user_id IS DISTINCT FROM v_authenticated_user_id THEN
    RAISE EXCEPTION 'User ID mismatch: cannot create or update organization for another user';
  END IF;
