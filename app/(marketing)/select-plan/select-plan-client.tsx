...
import { Check, ArrowRight, Loader2, Shield, AlertTriangle, CheckCircle2, Clock } from 'lucide-react';
...
const SOLO_PRICE_ID =
  (process.env.NEXT_PUBLIC_STRIPE_SOLO_PRICE_ID || 'price_1TEHcrFjJxHsNvNGmvH3pQur').replace(/\s+/g, '');
const PRACTICE_PRICE_ID =
  (process.env.NEXT_PUBLIC_STRIPE_PRACTICE_PRICE_ID || 'price_1TEHd6FjJxHsNvNGahdVbS6N').replace(/\s+/g, '');
const CLINIC_PRICE_ID =
  (process.env.NEXT_PUBLIC_STRIPE_CLINIC_PRICE_ID || 'price_1TEHdcFjJxHsNvNGzViIgMp8').replace(/\s+/g, '');
const ENTERPRISE_PRICE_ID =
  (process.env.NEXT_PUBLIC_STRIPE_ENTERPRISE_PRICE_ID || 'price_1TEHdcFjJxHsNvNGzViIgMp8').replace(/\s+/g, '');
...
export interface TrialContext {
  subscriptionStatus: 'trial' | 'expired' | null;
  trialDaysRemaining: number | null;
  policiesDrafted: number;
  riskAssessmentComplete: boolean;
  evidenceCount: number;
  complianceScore: number;
}

function TrialContextBanner({ ctx }: { ctx: TrialContext }) {
  if (!ctx.subscriptionStatus) return null;

  const isExpired = ctx.subscriptionStatus === 'expired';

  const headline = isExpired
    ? 'Your trial has ended. Your data is safe — subscribe to unlock exports and finalize your compliance documentation.'
    : ctx.trialDaysRemaining !== null && ctx.trialDaysRemaining <= 1
      ? 'Your free trial expires tomorrow. Upgrade now to keep your compliance work.'
      : `Your free trial ends in ${ctx.trialDaysRemaining} day${ctx.trialDaysRemaining !== 1 ? 's' : ''}. Subscribe now to activate exports, downloads, and policy finalization.`;

  const items = [
    {
      label: `${ctx.policiesDrafted} of 9 policies drafted`,
      done: ctx.policiesDrafted > 0,
    },
    {
      label: `Risk Assessment: ${ctx.riskAssessmentComplete ? 'complete' : 'not started'}`,
      done: ctx.riskAssessmentComplete,
    },
    {
      label: `${ctx.evidenceCount} document${ctx.evidenceCount !== 1 ? 's' : ''} in Evidence Center`,
      done: ctx.evidenceCount > 0,
    },
    {
      label: `Compliance score: ${ctx.complianceScore}/100`,
      done: ctx.complianceScore > 0,
    },
  ];

  return (
    <div className="w-full max-w-3xl mx-auto mb-10">
      <div className={`px-6 py-4 flex items-start gap-3 ${isExpired ? 'bg-red-50 border border-red-200' : 'bg-[#0e274e]'}`}>
        <AlertTriangle className={`h-5 w-5 shrink-0 mt-0.5 ${isExpired ? 'text-red-500' : 'text-[#00bceb]'}`} />
        <p className={`text-sm font-light ${isExpired ? 'text-red-800' : 'text-white'}`}>{headline}</p>
      </div>

      <div className={`px-6 py-4 border-t-0 ${isExpired ? 'bg-red-50/50 border border-red-200' : 'bg-[#0e274e]/10 border border-[#0e274e]/20'}`}>
        <p className="text-xs font-medium text-[#0e274e] mb-3 uppercase tracking-wider">
          You&apos;ve already set up:
        </p>
        <ul className="space-y-1.5">
          {items.map((item, i) => (
            <li key={i} className="flex items-center gap-2 text-sm text-[#565656] font-light">
              {item.done ? (
...
export default function SelectPlanClient({ trialContext }: SelectPlanClientProps) {
  return (
    <div className="min-h-screen bg-[#f3f5f9] flex flex-col items-center justify-center px-4 py-16">
      ...
      {/* Trial/Expired Contextual Banner */}
      {trialContext && <TrialContextBanner ctx={trialContext} />}

      {/* Header */}
      <div className="text-center mb-12 max-w-xl">
        <div className="inline-flex items-center gap-2 text-[#00bceb] mb-4">
          <Shield size={18} />
          <span className="text-sm font-thin tracking-wide uppercase">
            {trialContext ? 'Upgrade your plan' : 'Account Created'}
          </span>
        </div>
