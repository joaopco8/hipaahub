-- ============================================================================
-- CORREÇÃO DEFINITIVA: Erro "null value in column state"
-- ============================================================================
-- Execute este script COMPLETO no Supabase SQL Editor
-- ============================================================================

-- ============================================================================
-- PASSO 1: ADICIONAR VALOR PADRÃO NO BANCO DE DADOS (Defesa em Profundidade)
-- ============================================================================
-- Isso garante que mesmo se NULL chegar, o banco usa um valor padrão
ALTER TABLE public.organizations 
ALTER COLUMN state SET DEFAULT 'CA';

-- Se já existirem registros com NULL, atualizar
UPDATE public.organizations 
SET state = 'CA' 
WHERE state IS NULL;

-- Garantir que a constraint NOT NULL ainda existe (caso tenha sido removida)
ALTER TABLE public.organizations 
ALTER COLUMN state SET NOT NULL;

-- ============================================================================
-- PASSO 2: REMOVER FUNÇÃO ATUAL
-- ============================================================================
DROP FUNCTION IF EXISTS upsert_organization_jsonb(jsonb) CASCADE;

-- ============================================================================
-- PASSO 3: CRIAR FUNÇÃO COM TRATAMENTO ROBUSTO DE NULL
-- ============================================================================
...
DECLARE
  ...
  v_state text;  -- Variável para garantir que state nunca seja NULL
BEGIN
  v_user_id := auth.uid();
  ...
  -- ========================================================================
  -- GARANTIR QUE STATE NUNCA SEJA NULL (MÚLTIPLOS FALLBACKS)
  -- ========================================================================
  v_state := COALESCE(
    NULLIF(p_data->>'state', ''),
    NULLIF(p_data->>'address_state', ''),
    'CA'  -- Valor padrão final
  );
  ...
  IF v_existing_org_id IS NULL THEN
    ...
    VALUES (
      ...
      v_state,  -- Usar variável garantida não-NULL
  ...
  ELSE
    ...
    SET
      ...
      state = COALESCE(v_state, o.state),  -- Usar variável garantida não-NULL, ou preservar existente
...
-- Verificar que a coluna state tem DEFAULT
SELECT 
  '✅ Coluna state' as status,
  column_name,
  column_default,
  is_nullable
