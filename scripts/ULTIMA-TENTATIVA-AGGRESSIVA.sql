-- ============================================================================
-- ÚLTIMA TENTATIVA AGRESSIVA - Execute este script COMPLETO
-- ============================================================================
-- Este script resolve o erro "operator does not exist: text = uuid" de forma DEFINITIVA
-- ============================================================================

-- ============================================================================
-- PASSO 1: DIAGNÓSTICO - Ver EXATAMENTE o que existe
-- ============================================================================
SELECT 
...
-- ============================================================================
-- PASSO 2: REMOVER ABSOLUTAMENTE TUDO (DE TODOS OS SCHEMAS)
-- ============================================================================
DO $$
DECLARE
  r RECORD;
...
BEGIN
  RAISE NOTICE '=== REMOVENDO TODAS AS FUNÇÕES (TODOS OS SCHEMAS) ===';
  
  FOR r IN 
    SELECT n.nspname as schema_name, p.proname, pg_get_function_arguments(p.oid) as args, p.oid
    FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE p.proname LIKE '%upsert%organization%'
       OR p.proname LIKE '%update%organization%extended%'
  ...
  LOOP
    BEGIN
      ...
      RAISE NOTICE '✅ Removida: %.%(%)', r.schema_name, r.proname, r.args;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE '⚠️ Erro ao remover %.%(%): %', r.schema_name, r.proname, r.args, SQLERRM;
    END;
  END LOOP;
...
-- Remover explicitamente de public
...
-- Se ainda houver funções, listar
SELECT 
  'FUNÇÕES RESTANTES' as etapa,
  n.nspname as schema_name,
  p.proname as function_name,
  pg_get_function_arguments(p.oid) as arguments
FROM pg_proc p
...
-- ============================================================================
-- PASSO 4: CRIAR FUNÇÃO CORRETA (SEM TEXT, SEM UUID NA ASSINATURA)
-- ============================================================================
...
-- ============================================================================
-- PASSO 6: FORÇAR REFRESH DO CACHE (MÚLTIPLAS VEZES COM DELAY)
-- ============================================================================
...
SELECT '⚠️ IMPORTANTE: Se ainda der erro, REINICIE o PostgREST no Supabase Dashboard' as proximo_passo;
